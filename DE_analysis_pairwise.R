...
t1_t2 <- results(dds_wald, contrast=c("group", "t2", "t1"))
...
t2_t3 <- results(dds_wald, contrast=c("group", "t3", "t2"))
...
t3_t4 <- results(dds_wald, contrast=c("group", "t4", "t3"))
...
t1_t3 <- results(dds_wald, contrast=c("group", "t3", "t1"))
rownames(t1_t3) <- rownames(count_matrix_anno)

t1_t4 <- results(dds_wald, contrast=c("group", "t4", "t1"))
rownames(t1_t4) <- rownames(count_matrix_anno)

t2_t4 <- results(dds_wald, contrast=c("group", "t4", "t2"))
